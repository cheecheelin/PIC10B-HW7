...
/**
   To deep_copy: 
   - Pre-order traverse sourceTree and insert nodes into 'this'
*/
template <typename T>
...
bool BSTree<T>::operator==( const BSTree<T>& tree ) const {
    //if to_vector==to_vector
    std::vector<T> v1(0);
    std::vector<T> v2(0);
    to_vector(this->get_root(), v1);
    to_vector(tree.get_root(), v2);
    
    for (int i=0; i<v1.size(); i++){
        if (v1[i]!=v2[i])
            return false;
    }
    return true; // <-- Arbitrary return
}


//done
template <typename T>
void BSTree<T>::to_vector( TreeNode<T>* node , vector<T>& v ) const {
    if(node==NULL)
        return;
...
template <typename T>
ostream& operator<<( ostream& out , const BSTree<T>& tree ) {
    std::vector<T> emptyvector(0);
    tree.to_vector(tree.get_root(),emptyvector);
